...
void ssd1306_command(uint8_t c)
{
    ...
    uint8_t data[2]={control, c};
    ret_code_t ret = nrf_drv_twi_tx(&m_twi, _i2caddr, data, 2, false);
}
...
void SSD1306_Display(void)
{
  ...
  for (uint16_t i = 0; i < (SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT / 8); i+=16) {
      ...
      uint8_t data[17]; //transf 16 octeti plus inca unul
      data[0]=0x40;
      for(uint8_t j=0; j<16;j++)
        data[j+1]=ssd1306_buffer[i+j];
        ret_code_t ret = nrf_drv_twi_tx(&m_twi, _i2caddr, data, 17, false);     
